...
export class Simulation
{
    ...
    static MODE_ICO_DROP_10 = "icodrop10";
    static MODE_ICO_DROP_20 = "icodrop20";
    static MODE_SELL_100 = "sell100";

    public run(mode)
    {
        const TOTAL_EMISSION = 2000000000; // as stated in BitRewards WhitePaper
        const TOTAL_ICO_DISTRIBUTION = TOTAL_EMISSION * 0.64; // as stated in BitRewards WhitePaper

        const TOTAL_ICO_SALE_TOKENS = 614400000; // roughly USD 5.3kk, https://docs.google.com/spreadsheets/d/1l1gE5nLQ4AoNyAnjgULpJrCPTot9_C9TxNWMzY94u-I/edit#gid=0
        const ICO_BASE_PRICE = 0.00003472222222; // https://docs.google.com/spreadsheets/d/1l1gE5nLQ4AoNyAnjgULpJrCPTot9_C9TxNWMzY94u-I/edit#gid=0
        const MIN_PRICE = ICO_BASE_PRICE; // Exchanger will do its best to forbid price drop below that level
        const MIN_PRICE_ETH_RESERVE = 4200; // we put roughly USD 1.3kk (4200 ETH) to enforce that price doesn't drop below MIN_PRICE
         ...
         */
        const TOTAL_TOKENS_ON_MARKET_ON_START = TOTAL_ICO_SALE_TOKENS;

        /**
         * ETH amount that we put into Bancor Exchanger initially (1000 ETH ≈ $300'000).
         */
        const ETH_RESERVE_ON_EXCHANGER = TOTAL_TOKENS_ON_MARKET_ON_START * ICO_BASE_PRICE * Exchanger.TOKEN_RESERVE_RATIO;

        console.log(`ETH Reserve on exchanger: ${ETH_RESERVE_ON_EXCHANGER} ETH`);

        const MONTHS_COUNT = 60;
         ...
         */
        let TOKEN_MONTHLY_USD_ACQUISITION_BY_SHOPS = [
         ...
         */
        const TOKEN_USERS_SELL_DELAYED_FRACTIONS_BY_MONTH = mode == Simulation.MODE_SELL_100 ? [1] : [
         ...
         */
        let exchanger = new Exchanger(ETH_RESERVE_ON_EXCHANGER, TOTAL_TOKENS_ON_MARKET_ON_START, MIN_PRICE, MIN_PRICE_ETH_RESERVE);

        let initialPrice = exchanger.getCurrentTokenPrice();
        console.log(`Initial price: 1 BIT = ${initialPrice} ETH`);

        /**
         * History of BIT's amounts bought by shops for rewarding users each month
         */
        let bitAmountsHistory = [];

        const OUTPUT_FOR_CHARTS = true;
        ...
        for (let i = 0; i < MONTHS_COUNT; i++) {
            ...
            let usdBuyAmount = TOKEN_MONTHLY_USD_ACQUISITION_BY_SHOPS[i];
            let ethBuyAmount = Exchanger.usdToEth(usdBuyAmount);
            let bitBuyAmount = exchanger.buyTokensByEtherAmount(ethBuyAmount);
            let effectiveBitPrice = ethBuyAmount / bitBuyAmount;

            bitAmountsHistory.push(bitBuyAmount);

            let shopsCount = MONTHLY_SHOPS_COUNT[i];
            ...
            let bitSellAmount = 0;
            for (let j = 0; j < TOKEN_USERS_SELL_DELAYED_FRACTIONS_BY_MONTH.length; j++) {
                let month = i - j;
                if (month < 0) {
                    break;
                }
            ...
            let ethSellAmount = exchanger.sellTokens(bitSellAmount);
            ...
            if (!OUTPUT_FOR_CHARTS) {
                ...
                console.log(`Exchanger reserve: ${exchanger.getTotalEthReserve().toFixed(2)} ETH ($${Exchanger.ethToUsd(exchanger.getTotalEthReserve())})`);
            }


            let growthPercentage = Math.round((effectiveBitPrice - initialPrice) / initialPrice * 100);
